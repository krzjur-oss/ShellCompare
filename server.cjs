...
app.post("/api/translate-command", async (req, res) => {
  try {
    ...
    const systemInstruction = `Jeste\u015B ekspertem system\xF3w operacyjnych (Linux, Windows) oraz pow\u0142ok systemowych: Bash (Linux/macOS), CMD (klasyczny wiersz polece\u0144 Windows) oraz PowerShell (nowoczesny shell obiektowy).
...
Dodatkowo okre\u015Bl 'detectedSource' (np. 'Bash', 'CMD', 'PowerShell', 'Opis s\u0142owny') oraz stw\xF3rz 'comparisonMarkdown' zawieraj\u0105cy rzetelne por\xF3wnanie r\xF3\u017Cnic w sk\u0142adni, filozofii (np. obiekty vs strumienie tekstowe), obs\u0142udze b\u0142\u0119d\xF3w lub potencjalnych pu\u0142apkach (np. wielko\u015B\u0107 liter, aliasy) dla tej konkretnej operacji.
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            detectedSource: { type: import_genai.Type.STRING, description: "Automatycznie wykryte \u017Ar\xF3d\u0142o (np. Bash, CMD, PowerShell, Opis s\u0142owny)" },
...
app.post("/api/compare-concept", async (req, res) => {
  try {
    ...
    const ai = getGeminiClient();
    const systemInstruction = `Jeste\u015B wybitnym pedagogiem i administratorem system\xF3w operacyjnych. Wyja\u015Bnij poj\u0119cie techniczne/koncepcyjne podane przez u\u017Cytkownika w kontek\u015Bcie trzech \u015Brodowisk terminalowych: Bash, CMD i PowerShell.
...
- comparisonTable: obiekt z kluczami (kryterium por\xF3wnawcze) i opisem r\xF3\u017Cnic dla ka\u017Cdego systemu w formie zwi\u0119z\u0142ej tabeli lub punkt\xF3w.
